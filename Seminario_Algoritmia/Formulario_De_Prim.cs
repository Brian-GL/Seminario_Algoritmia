...
{
	/// <summary>
	/// Description of Formulario_De_Prim.
	/// </summary>
	public partial class Formulario_De_Prim : Form
	{
		Graph<Circulo> myCircleGraph = new Graph<Circulo>();
		Graph<Circulo> myPrimGraph;
		List<Triplet<Circulo,double,Circulo>> listaPrim;
		int numeroBosques;
		List<int> agentList = new List<int>();
		int cebo;
		Bitmap bitmapFondo, bitmapFrente;
		
		public Formulario_De_Prim(Graph<Circulo> PrimGraph, Bitmap bmp)
		{
			...
			myCircleGraph = PrimGraph;
		
			pictureBoxPrim.BackgroundImage = bitmapFondo;
			pictureBoxPrim.BackgroundImageLayout = ImageLayout.Zoom;
			
			for(int i = 0; i < PrimGraph.Size_Nodes();i++){
				comboBoxNodos.Items.Add(PrimGraph[i].data.ToString());
				comboBoxPrimerAgente.Items.Add(PrimGraph[i].data.ToString());
				comboBoxSegundoAgente.Items.Add(PrimGraph[i].data.ToString());
			}
			

			cebo = -1;
			
			DrawWeight(PrimGraph);
			dgvGraph.Sort(Weight,System.ComponentModel.ListSortDirection.Ascending);
		}

		
		void DrawWeight(Graph<Circulo> graph){
			var graficos = Graphics.FromImage(pictureBoxPrim.BackgroundImage);
		...
		void ButtonAnimarKruskalClick(object sender, EventArgs e)
		{
			...
			if(comboBoxNodos.Text.Length > 0){
				...
				relojLista.Start();
				listaPrim =  myCircleGraph.PrimFullList(GetCirculo(int.Parse(comboBoxNodos.SelectedItem.ToString())));
				relojLista.Stop();
				TimeSpan tiempoLista = relojLista.Elapsed;
				numeroBosques = myCircleGraph.NumeroDeBosques();
				myPrimGraph = Graph<Circulo>.CreateGraph(listaPrim);
				...
				for(int i = 0; i < listaPrim.Count;i++){
					...
					DrawARM(listaPrim[i].firstData,listaPrim[i].secondData,listaPrim[i].thirdData);
					suma+=listaPrim[i].secondData;
					Thread.Sleep(1000);
					int renglon = dgvGraph.Rows.Add();
					dgvGraph.Rows[renglon].Cells["Origin"].Value = listaPrim[i].firstData.ToString();
					dgvGraph.Rows[renglon].Cells["Weight"].Value =  listaPrim[i].secondData;
					dgvGraph.Rows[renglon].Cells["Destination"].Value = listaPrim[i].thirdData;
		...
		{
			int number;
			var addCebo = new AddCebo(comboBoxNodos);
			addCebo.ShowDialog();
		...
		void ButtonSimularClick(object sender, EventArgs e)
		{
			...
			if(agentList.Count == 1){
				...
				var rutaProfundidad = myPrimGraph.Depth_Travel_Total(GetCirculo(agentList.First()),circuloCebo);
				...
				var rutaProfundidad = myPrimGraph.Depth_Travel_Total(circuloPrimero,circuloCebo);
				var rutaAnchura = myPrimGraph.Width_Travel_Total(circuloSegundo,circuloCebo);
		...
		void Draw_Path(Circulo c1){
			var grafics = Graphics.FromImage(pictureBoxPrim.BackgroundImage);
